...
export const ProductCard = ({product}) => {
    const {id, name, overview, poster, price, image_local, rating, best_seller} = product;
  return (
    <div className="m-3 grid max-w-[23rem] bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
        <Link to={`products/${id}`} className="relative" >
         {best_seller && best_seller ? <span className="absolute top-4 left-2 px-2 bg-orange-500 bg-opacity-90 text-white rounded">Best Seller</span> : <span className="absolute top-4 left-2 px-2 bg-red-500 bg-opacity-90 text-white rounded">Not Best Seller</span>}   
